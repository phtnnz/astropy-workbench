...
#       Allow dict rows, automatically use DictWriter
...
class CSVOutput:
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        row={ data1: v1, data2: v2, date3: v3, ... }
        ...
        fields = kwargs.get("fields") or kwargs.get("fieldnames")
        if fields:
    ...
    def _write(self, f: TextIO) -> None:
        ...
        if use_dict:
            ...
            if locale.localeconv()['decimal_point'] == ",":
                ...
                writer = csv.DictWriter(f, dialect="excel", delimiter=";", quoting=csv.QUOTE_ALL,
                                        fieldnames=self._fields)
            else:
                writer = csv.DictWriter(f, dialect="excel", fieldnames=self._fields)
            writer.writeheader()
        else:
            # List     
            if locale.localeconv()['decimal_point'] == ",":
        ...
        for row in self._cache:
            if self._float_fmt:
                if use_dict:
                    ##FIXME: float conversion for dict
                    ...
                else:
